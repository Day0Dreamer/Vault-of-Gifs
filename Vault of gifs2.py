...
class QtMainWindow(QtGui.QMainWindow, MainWindow_UI.Ui_MainWindow):
    ...
    export_stage1 = QtCore.Signal()
    export_stage2_signal = QtCore.Signal()
    ...
    def __init__(self):
        ...
        self.export_stage2_signal.connect(self.export_stage2)
        ...
        def avi_double_clicked(video_list_item):
            if not video_list_item.data(ITEM_EMOJI_OBJECT).has_raw_gif:
                ...
                self.ffmpeg = FFmpeg()
                self.ffmpeg.add(video_list_item.data(ITEM_FULL_PATH), video_list_item.data(ITEM_FPS))
                self.ffmpeg.run()
                self.working_file = video_list_item.data(ITEM_FULL_PATH)
                self.update_video_list(self.working_directory)
            else:
...
if __name__ == '__main__':
    ...
    if args:
        MainWindowObj.export_folder(args.folder, args.ext, args.l280, args.l136, args.color)
